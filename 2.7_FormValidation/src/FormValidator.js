...
class FormValidator extends React.Component {
  constructor(props) {
    super(props);
    //ประกาศ Object ที่มี value เป็น Array ของ Object  
  ...
  handleFieldChange = e => {
    ...
    newState[e.target.name].value = e.target.value; // set ค่า state ใหม่จาก <input> ที่เรียกใช้ว่ามี attribute name เป็นค่าอะไร 
    this.validateForm(newState);
  };

  handleSetTouched = e => {
    let field = { ...this.state[e.target.name], isTouched: true }; //state ตัวไหนที่โดนแตะแล้วจะถูกแก้เป็น true
    this.setState({ [e.target.name]: { ...field } }); //ตรงนี้จะไม่แก้ค่า isValid กับ error
  };
